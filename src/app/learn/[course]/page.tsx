            ...
            <div className="mt-10 grid gap-3">
              {course.lessons.map((lesson, index) => (
                <Link key={lesson.slug} href={`/learn/${course.slug}/${lesson.slug}`} className="surface group flex items-center gap-4 rounded-2xl p-5 transition hover:-translate-y-0.5 hover:border-brand-300">
                  ...
                  <div className="min-w-0 flex-1"><h2 className="font-black text-slate-950 dark:text-white">{lesson.title}</h2><p className="mt-1 text-sm text-slate-500 dark:text-slate-400">{lesson.summary}</p></div>
